...
Here's what IATI data looks like in a raw format:
...
In terms of the tool, we might want to filter by organization type, e.g. we don't want to include government departments.
They will be referring down to a partner level.
...
Check out [companieshouse.gov.uk](https://companieshouse.gov.uk)
Identifiers are used to link organisation within datasets.

## What is an activity?
Link out to the registry, which is where the data sits. XML on a bunch of files.
You might a bunch of activities on a single file.
...
Currently, the latest data format is 2.03.
...
This new datastore is being built by Zimmerman Zimmerman (TODO: link to lower down) and is called IATI.cloud (TODO: link to lower).
[Description of moving to new Datastore](https://iatistandard.org/en/using-data/iati-tools-and-resources/iati-datastore/)
...
can provide some kind of structural validation. Doesn't do semantic validation.
...
This tool is opt-in. So while this application is capable of 
...
TODO: show the code without running it
...
## Individuals and Code

* [IATI analysis scripts](https://github.com/rorads/Analyst-IATI-Collab)
...
-> creating the data
-> doing QA on the data that we have publishde
-> interrogating the data that has been published
-> tools that exist to help people creae and publish their data
