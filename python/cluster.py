...
def apply_svd(term_document_matrix_dataframe):
    ...
    svd_term_document_matrix_dataframe = svd.fit_transform(term_document_matrix_dataframe)
    end = time.time()
...
def kmeans_clustering(term_document_matrix_dataframe, term_dataframe, minimum_number_of_clusters, maximum_number_of_clusters, increment):
    ...
    for n_clust in range(minimum_number_of_clusters, maximum_number_of_clusters, increment):
        ...
        append_to_csv(join(get_data_path(), CLUSTERING_SUM_OF_SQUARE_COMPARISON_FILENAME), [n_clust, clusters.inertia_])

        #Write cluster assigned to each iati.identifier out to csv
        term_dataframe[['iati.identifier', 'cluster{0}'.format(n_clust)]].to_csv(join(get_data_path(),ACTIVITY_CLUSTER_ASSIGNMENT_FILENAME_CONVENTION.format(n_clust)), encoding='iso-8859-1', index=False)
...
if __name__ == "__main__":
    ...
    term_dataframe = pd.read_csv(join(get_data_path(), PROCESSED_RECORDS_FILENAME), encoding='iso-8859-1') 
